...
def user_creation_page():
    with st.container() as metrics_section:
        ...
        #st.markdown("<span style='display: inline-block;border-right: 1px solid #ccc;margin: 0 10px;height: 1000px; padding: 10px;'></span>",unsafe_allow_html=True)
    ...
    #     styles={
    #     "container": {"padding": "0!important", "background-color": "#fafafa","top":"left"},
    #     "nav-link": {"font-size": "15px", "text-align": "left", "margin":"0px", "--hover-color": "#eee"},
    #     "nav-link-selected": {"background-color": "#6D7294"},
    # }
    # )
...
def revoke_role():
    ...
    Revoke_Roles = f"""
        <button style="width: 50%; height: 35px; margin-top:20px; background: linear-gradient(to right, #a02a41 0%,    #1D4077 100%); color: white; border-radius: 15px;">Revoke Roles</button>
        """
...
def role_assignment():
    ...
    Assign_Roles = f"""
        <button style="width: 50%; height: 35px; margin-top:20px; background: linear-gradient(to right, #a02a41 0%,    #1D4077 100%); color: white; border-radius: 15px;">Assign Roles</button>
        """
