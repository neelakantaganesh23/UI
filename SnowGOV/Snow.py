...
image = Image.open('SnowGOV/SnowGov.png')
...
with open ('SnowGOV/styles.css') as f:
...
def database_management():
    ...
    if choose == 'Database':
        ...
        with st.form(key='my_form2'):
            ...
            environment = st.radio('**ENVIRONMENT :**', ['DEV', 'PROD', 'STAGE', 'TEST'])

            #col1 , col2, col3 ,col4 = st.columns([1,1,1,1])

            #with col1:
                #st.radio('Enviroment:', ['Dev'])
            #with col2:
                #st.radio(' ', ['Prod'])
            #with col3:
                #st.radio(' ', ['Stage'])
...
def user_creation_page():
    not_required = option_menu(
        ...
        styles={
        "container": {"padding": "0!important", "background-color": "#fafafa","top":"left","font-family": "Poppins"},
        "nav-link": {"font-size": "15px", "text-align": "left", "margin":"0px", "--hover-color": "#eee","font-family": "Poppins"},
        "nav-link-selected": {"background-color": "#6D7294","font-family": "Poppins"},
    }
    )
...
def role_manage():
    role_choice = option_menu(
        ...
        menu_icon = 'person-fill-gear',
         styles={
        "container": {"padding": "0!important", "background-color": "#fafafa","font-family": "Poppins"},
        "nav-link": {"font-size": "15px", "text-align": "left","font-family": "Poppins", "margin":"10px", "--hover-color": "#eee"},
        "nav-link-selected": {"background-color": "#6D7294","font-family": "Poppins"},
    }
    )
...
def revoke_role():
    ...
    Revoke_Roles = f""" <div style="display:flex;justify-content:center;">
        <button style="font-family:Poppins;font-family: Poppins;padding: 10px 30px;margin-bottom:20px; background: linear-gradient(to right, #a02a41 0%,#1D4077 100%); color: white; border-radius: 50px;">Revoke Roles</button></div>
        """
    if st.markdown(Revoke_Roles,unsafe_allow_html=True):
        try:
...
def role_assignment():
    ...
    Assign_Roles = f"""
         <div style="display:flex;justify-content:center;"> <button style=" text-align :center;  background: linear-gradient(to right, #a02a41 0%,    #1D4077 100%); color: white; border-radius: 50px; padding:10px 30px;">Assign Roles</button></div>
        """
    if st.markdown(Assign_Roles,unsafe_allow_html=True):
        try:
